  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      if (ADC_Process())
      {
        ...
        // 判断外部输入电压（底盘电压）是否够高
        // 如果电源被移除又重新接上，在这断电的停顿瞬间如果强行运行充电 Buck (A侧下管导通) 会变成 Boost 从电容剧烈反灌，触发25A保护
        if (RE_V_CHASSIS <= RE_V_CAP + 0.3f)
        {
          // 电源尚未就绪或电压不足，关闭 PWM 输出以确保安全等待
          HAL_HRTIM_WaveformOutputStop(&hhrtim1, HRTIM_OUTPUT_TA1 | HRTIM_OUTPUT_TA2 | HRTIM_OUTPUT_TB1 | HRTIM_OUTPUT_TB2);
          PID_InitAll(); // 停机期间不要积分
          current_duty = 0.0f;
        }
        else
        {
      ...
      while (1)
      {
        ...
        if (ADC_Process())
        {
          ...
          if (current_duty_dis < 0.2f)
            current_duty_dis = 0.2f;
          if (current_duty_dis > 1.0f)
            current_duty_dis = 1.0f;
